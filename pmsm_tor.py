...
def solve_pmsm(outdir: Path = Path("results"), plot: bool = False, progress: bool = False, save_output: bool = False):
    ...
    if mesh.comm.rank == 0:
        print(f"{omega_u}, {avg_torque}, {avg_vol_torque}, {RMS_Voltage}, {pec_tot_p}, {pec_steel_p}, {freq}, {degree}, {elements}, {num_dofs} ")
    ...
    if mesh.comm.rank == 0 and plot:
        plt.figure()
        plt.plot(times, torques, "--r", label=f"Surface Torque, Avg. = {avg_torque:.2f} N.m")
        plt.plot(times, torques_vol, "-b", label=f"Volume Torque, Avg. = {avg_vol_torque:.2f} N.m")
        plt.plot(times[last_period], torque_v_p, "--g")
        plt.title(f"Torque vs Time for 600 RPM ({omega_u} rad/s) \n", fontsize=14)
        # plt.plot([], [], ' ', label=f"Avg. Surface Torque = {avg_torque:.2f} N.m")
        # plt.plot([], [], ' ', label=f"Avg. Volume Torque = {avg_vol_torque:.2f} N.m")
        plt.xlabel('Time (s)')
