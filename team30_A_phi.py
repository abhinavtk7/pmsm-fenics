...
def solve_team30(single_phase: bool, num_phases: int, omega_u: np.float64, degree: np.int32, petsc_options: dict = {},
    ...
    # currents = {7: {'alpha': 1, 'beta': 0}, 8: {'alpha': -1, 'beta': 0}}
    ...
    for i in range(num_steps):
        ...
        # Az_out1 = AzV.sub(0).collapse()
        # print(np.unique(Az_out1.x.array))
        # # Post-processing function for projecting the magnetic field potential
        # post_B1 = MagneticField2D(AzV)
        # print(np.unique(post_B1.B.x.array))
...
if __name__ == "__main__":
    ...
    if args.single:
        outdir = Path(f"Dec23_TEAM30_single_{args.omegaU}_{args.degree}_{args.steps}_{args.apply_torque}")
    ...
    if args.three:
        outdir = Path(f"Dec23_TEAM30_three_{args.omegaU}_{args.degree}_{args.steps}_{args.apply_torque}")
        outdir.mkdir(exist_ok=True)
