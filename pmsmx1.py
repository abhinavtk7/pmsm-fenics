...
# from dolfinx import ( Function, FunctionSpace, log,     #DirichletBC,
...
def SumFunctions(fncspace, name, A, B):
    fnc = dolfinx.fem.Function(fncspace)
    fnc.name = name
    fnc.vector.set(0.0)
...
# Mesh File Name
# meshname = "pmesh3D.xdmf"
meshname = Path("meshes") / "pmesh3D.xdmf"
# Frequency (Hz)
freq = 50.0
...
filedir = os.getcwd()
meshpath = os.path.join(filedir, meshname)
if MPI.COMM_WORLD.rank == 0: 
...
with XDMFFile(MPI.COMM_WORLD,
    ...
    mt_domains = file.read_meshtags(mesh, name="Cell_markers")    #file.read_meshtags(mesh, "domains")
    tdim = mesh.topology.dim        # 2
    mesh.topology.create_connectivity(tdim - 1, 0)
    mt_facets = file.read_meshtags(mesh, name="Facet_markers") 
...
V_V    = dolfinx.fem.FunctionSpace(mesh, CG_V)
V_VF   = dolfinx.fem.FunctionSpace(mesh, CG_VF)
...
jsexp.omega = omega_f
jsource = dolfinx.fem.Function(V_V)
...
msource = dolfinx.fem.Function(V_V)
...
steel_domain       = (dx(4) + dx(5) + dx(6))
coil_domain        = (dx(7) + dx(8) + dx(9) + \
                      dx(10) + dx(11) + dx(12))
magnet_domain      = (dx(13) + dx(14) + dx(15) + dx(16) + \
                      dx(17) + dx(18) + dx(19) + dx(20) + \
                      dx(21) + dx(22))
air_domain         = (dx(1) + dx(2) + dx(3))

rot_steel_domain   = (dx(4) + dx(5))
...
bcs_av.append( dolfinx.fem.dirichletbc(u0.sub(0).sub(1), x_faces) )
bcs_av.append( dolfinx.fem.dirichletbc(u0.sub(0).sub(2), x_faces) )
bcs_av.append( dolfinx.fem.dirichletbc(u0.sub(0).sub(0), y_faces) )
bcs_av.append( dolfinx.fem.dirichletbc(u0.sub(0).sub(2), y_faces) )
bcs_av.append( dolfinx.fem.dirichletbc(u0.sub(0).sub(0), z_faces) )
bcs_av.append( dolfinx.fem.dirichletbc(u0.sub(0).sub(1), z_faces) )
...
# A & V Solver
# cpp_a = dolfinx.fem.form(a, form_compiler_options=form_compiler_options,
#                      jit_options=jit_parameters)
# pattern = dolfinx.fem.create_sparsity_pattern(cpp_a)

A_av = dolfinx.fem.petsc.assemble_matrix(dolfinx.fem.form(a_av), bcs_av)
A_av.assemble()
AV = dolfinx.fem.Function(V_VF)
...
#     if MPI.COMM_WORLD.rank == 0: log.log(loglevel, "T: Calculating")
...
t_run.stop()
dolfinx.list_timings(mesh.comm, [dolfinx.TimingType.wall])