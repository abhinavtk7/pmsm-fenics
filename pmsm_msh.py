...
def solve_pmsm(res: int, outdir: Path = Path("results"), progress: bool = False, save_output: bool = False ):
    ...
    if not apply_torque:
        ...
        A_np = A.convert("dense")  # Convert to dense format
        A_cond = np.array(A_np[:, :])
        # Compute condition number using SVD
        sigma_max = np.linalg.norm(A_cond, 2)  # Largest singular value
        sigma_min = np.linalg.norm(A_cond, -2)  # Smallest singular value
        cond_number = sigma_max / sigma_min
        # from slepc4py import SLEPc  # Import SLEPc for eigenvalue computation

        # eigensolver = SLEPc.EPS().create()
        # eigensolver.setOperators(A)
        # eigensolver.setProblemType(SLEPc.EPS.ProblemType.HEP)  # Hermitian Eigenvalue Problem
        # eigensolver.setWhichEigenpairs(SLEPc.EPS.Which.LARGEST_MAGNITUDE)  # Find largest eigenvalue
        ...
        # if nconv > 1:
        #     sigma_max = eigensolver.getEigenvalue(0)  # Largest eigenvalue
        #     sigma_min = eigensolver.getEigenvalue(nconv - 1)  # Smallest eigenvalue
        #     cond_number = abs(sigma_max / sigma_min)
        #     print(f"Condition Number of Mesh: {cond_number}")
        # else:
        #     print("Failed to compute eigenvalues: Solver did not converge.")
